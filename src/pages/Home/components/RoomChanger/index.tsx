...
import { ROOM_DATA, ROOM_DATA_ARRAY } from '../../constants';
...
type CustomRoomData = VisibleRoom & {
  key: string;
  visible: boolean;
...
const RoomChanger = (props: RoomChangerProps) => {
  ...
  const handleFloorChange = useCallback(
    (floor: number) => {
      const currentFloorRoomList = ROOM_DATA_ARRAY.filter(
        (item) => item.floor === floor,
      );
      const indexRoom =
        currentFloorRoomList.find((item) => !!item.index) ||
        currentFloorRoomList[0];
      ...
      {roomList
        ...
        .map((room) => {
          ...
          return (
            <div
              key={key}
              className={`${markerStyle['room-marker']} ${styles['room-changer-item-visible']} ${styles['room-changer-item-common']} ${styles['room-changer-item-animate']}`}
              onClick={onRoomChange.bind(null, key)}
              style={extraStyle}
            >
              {name}
